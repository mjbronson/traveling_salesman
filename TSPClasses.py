...
class Scenario:
    ...
    def __init__(self, city_locations, difficulty, rand_seed):
        ...
        if difficulty == "Normal" or difficulty == "Hard":
            self.cities = [City(pt.x(), pt.y(), random.uniform(0.0, 1.0)) for pt in city_locations]
        ...
        self._edge_exists: np.array = (np.ones((ncities, ncities)) - np.diag(np.ones((ncities)))) > 0
